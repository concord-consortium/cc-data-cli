...
`reportUnionView` gains a parameter describing the columns to append; `reports` and `report_prompts` pass none and are byte-identical to today. The descriptor is a slice of `{name, typ, src, expr func(col string) string}`. `src` names the CSV column the output is derived from, and the descriptor itself substitutes `CAST(NULL AS <typ>)` for a member whose schema lacks it, so `typ` is declared once and cannot disagree with what the member emits. `expr` is then a pure expression builder over the quoted source identifier.
...
The zero-member short-circuit (`reportUnionView`'s zero-member short-circuit) emits `run_id` plus each derived column as a typed NULL rather than `run_id` alone. With an empty descriptor that is character-for-character today's statement, so `reports` and `report_prompts` are unaffected.

`perDownloadViews` is the third caller of `csvScan`/`csvEmptyMember` (`perDownloadViews`). It passes no derived columns, so `report_<run>` views keep exactly today's shape even for a log run. The run-scoped form of the parsed view is `logs` with `WHERE run_id = <id>`, which is the pairing the `reports` catalog entry already documents; giving `report_<run>` type-dependent columns would be a wider change than this story.
...
`epochExpr(perSecond)` returns `to_timestamp(TRY_CAST(<col> AS DOUBLE)) AT TIME ZONE 'UTC'`, dividing by `perSecond` first when it is not 1. Taking the divisor as a parameter rather than writing the two expressions out is what keeps the seconds and milliseconds readings from being independently editable. `jsonExpr` returns `TRY_CAST(<col> AS JSON)`.

Neither handles an absent source column, because the descriptor does it for them off `src`. That case is reachable: `recoverReportType` admits a CSV to the log type on `event` and `time` alone (`internal/dataset/reindex.go:336`), so a CSV with no `parameters` column can be a member, and referencing it would fail the view's creation for every other run in the dataset.

A member also skips a derived column whose name its own schema already contains. The consequence of omitting that skip depends on how many log CSVs the dataset holds, and only one of the two outcomes is survivable. With a single member there is no `UNION` keyword and `CREATE VIEW` accepts the duplicate, but not by leaving two columns of one name: DuckDB renames the second to **`event_time_1`**, so `SELECT event_time` resolves deterministically to the CSV's own column and the derived value is reachable only under a name nothing documents. With two or more, `UNION ALL BY NAME` rejects the duplicate outright, and since the `fallback` statement unions the same members it carries the same duplicate and fails identically, so `Open` returns an error and the **whole dataset** fails to register rather than one view (`internal/duck/engine.go:88-91`).
...
Registration goes after `reportPromptsView` in `statements()` (`statements`), which places `logs` next to the other report-derived views and inside what `StaticViewNames()` enumerates.
...
- **Every type `DetectCSV` can produce for an epoch column still works**: three runs in one dataset, with integral, non-numeric and fractional `time` values, so the column is detected `BIGINT`, `VARCHAR` and `DOUBLE`. The fixture asserts those three detected types before querying, or it would prove nothing. The view creates, the integral and fractional runs yield correct instants (the fractional one keeping its half second) and the non-numeric row is null. Fails if the `TRY_CAST(time AS DOUBLE)` is dropped, because `to_timestamp(VARCHAR)` is a binder error that takes the whole view down.
- **A member lacking `parameters` contributes**: a CSV with `event`, `time` and `extras` and neither `parameters` nor `timestamp`, so both typed-NULL branches are covered in one fixture. The view creates, that run's `parameters_json` and `received_time` are null, and its own columns still populate. Fails if the descriptor stops substituting a typed NULL for an absent `src`.
...
- **A source column named `event_time` is not duplicated, at both member counts**: with one log CSV carrying its own `event_time`, `logs` has exactly one column whose name *begins* `event_time`, and it keeps the CSV's own VARCHAR type. Counting columns named exactly `event_time` would not work: `CREATE VIEW` renames the second to `event_time_1`, so the exact name stays unique whether or not the skip ran, and the assertion has to be on the prefix. With a second log CSV the union is real, so the same fixture asserts that any rows come back at all; without the skip both the primary and the fallback hit `Binder Error: UNION (ALL) BY NAME operation doesn't support duplicate names` and `Open` fails for the whole dataset. One case without the other tests half the guard.
...
A second note: `TRY_CAST(... AS JSON)` needs no extension load. `attachmentContentView` already ships the same expression (`attachmentContentView`), so the JSON type is known to work in the embedded engine as configured.
