...
- The timing assertion needs a 1M-row fixture, roughly 455MB of generated JSONL, so it is opt-in through an environment variable rather than run on every CI job. It asserts a column-pruned aggregate at 10x against a measured 54x, and separately that a high-cardinality aggregate is not slower. Its fixture generator must emit varied payloads.
