...
Tests: materializing then querying returns identical rows and column types for every view in `MaterializableViews()`; a second run skips everything; `--force` rewrites; a view whose declared input is missing is refused while every other view is still written, and proceeds under `--allow-partial`; a view that fell back to typed-empty is refused with `--allow-partial` given; a store-backed Parquet short of `Stores[typ].Count` is refused; an incomplete download warns and blocks nothing; a `get` between the copy and the repoint leaves the view unmaterialized rather than wrongly fresh (driven through the same `testHookAfterWrite` seam `merge.go` uses); and no `.tmp-` file survives any of those paths.
