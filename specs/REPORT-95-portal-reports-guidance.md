...
- A guard holds that every report slug named in the guidance exists in the code's slug inventories.
...
- Measured on the finished implementation: core 17,018 bytes, skill 20,306, MCP instructions 20,519. This story added 2,684 / 3,400 / 2,797 against a baseline that had itself grown 39% since the spec was drafted. The skill grows most because it alone carries the command spellings the core is forbidden to name.
...
- **The four shipped catalogs need no new test for the regex widening.** The view, tool, identity-column and researcher-guide guards already compare each catalog against its code-derived set in both directions, so a name the widened pattern gained or lost fails one of them. A new test asserting the same comparison could not fail unless one of those four also failed.
