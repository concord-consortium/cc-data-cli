...
The rename resolves its target rather than assuming it is free. A CSV carrying both `event_time` and `event_time_source` would otherwise emit the target twice, and because views register when the dataset opens, that fails the primary and the fallback alike and leaves **every** query against the dataset dead, `SELECT 1` included. The suffix is reapplied until the name is unused. It is checked only against the CSV's own columns, not against targets other renames took, which is safe while no derived name is another with `_source` repeated on the end; that is worth re-checking if a fifth derived column is added.

Nothing in cc-data can detect the server-side change that would cause any of this: it holds no copy of the log column list and derives none, so a test asserting the four names against a transcript would fail only when a person edited the transcript. The rename is the whole defense, tested by behavior at one member, at two, on the stand-in, and against an already-taken target with the file both present and missing.
