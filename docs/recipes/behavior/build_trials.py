...
"stopped to look" -- 119,057 operation gaps decay smoothly with no second peak.
...
changing stretch is a trial. Measured on the real corpus: 406 of the 506
documents with a Simulator tile show such episodes, and 47.3% of episodes follow
...
COPY (
  ...
  seq AS (
    SELECT doc_id, created, v,
           lag(v) OVER (PARTITION BY doc_id ORDER BY created) AS prev_v
    FROM raw
  ),
  -- Only actual changes count. The slider sitting still rewrites the same value
  -- repeatedly, and that is not a trial.
  chg AS (
    SELECT doc_id, created FROM seq
    WHERE prev_v IS NOT NULL AND v IS DISTINCT FROM prev_v
  ),
  marked AS (
    SELECT *,
      CASE WHEN date_diff('second',
             lag(created) OVER (PARTITION BY doc_id ORDER BY created),
             created) > {gap_s}
        OR lag(created) OVER (PARTITION BY doc_id ORDER BY created) IS NULL
  ...
  grouped AS (
    SELECT *,
      ...
      CAST(sum(is_new) OVER (PARTITION BY doc_id ORDER BY created
                             ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
           AS BIGINT) AS trial_id
    FROM marked
  )
  SELECT doc_id, trial_id,
...
def main():
    ...
    """ % (os.path.join(derived, "edits.parquet"), out))[0]
    if coupled["n"]:
