...
- **Create a run** (`cc-data reports create`) from a filter assembled with `filter-options`, or take a fresh snapshot of an existing one (`cc-data reports duplicate`), so a workflow no longer has to start in the web form.
- **Download a run's data** into a dataset:
  - `get report <run-id>`: the report CSV. An Athena run polls until its query succeeds and then follows a presigned URL; a Portal run's CSV is computed and streamed on request. Neither writes a partial file.
...
cc-data reports filter-options --dimension <dimension> --portal <portal|env>   # values a filter offers; --search, --all, --json, --report-filter
cc-data reports create --report-slug <slug> --portal <portal|env>   # new run; --report-filter '{"cohort":[1]}' or --report-filter-file
cc-data reports duplicate <run-id> --portal <portal|env>   # fresh snapshot of a run; --force for a Portal run
